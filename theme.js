function igualarBlocos(cc){
    try{
            este = this;
            jt = jQuery(este);
            maisComprido = 0;	
            maisCurto = 99999;	
            jQuery(este).find(".block-outer").each(function () {
                    if(jQuery(this).height() > maisComprido)
                            maisComprido = jQuery(this).height();
                    if(jQuery(this).height() < maisCurto)
                            maisCurto = jQuery(this).height();	
            });
            var inner1=  jQuery(este).find(".block-inner-1");
            if(cc!=0){
                    jQuery(inner1).each(function (){ jQuery(this).height(maisComprido)});
                    jQuery(este).closest(".block-outer").height(jQuery(este).height() + maisComprido - maisCurto + 70 );
            }else{
                    //Iguala os inner 2 ao inner 1
                    jt.find(".block-inner-1 .block-inner-2").each(function (idx) {
                            if(idx==2)
                                    jQuery(this).height(315);	
            });
        }
    }catch(err){}
}
...
(function($) {
  ...
  $('.container-block-plugin_container-block').each(igualarBlocos);
